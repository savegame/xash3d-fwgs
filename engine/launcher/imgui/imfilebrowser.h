...
namespace ImGui
{
    ...
    public:
        ...
        // default is (700, 450)
        void SetWindowSize(const ImVec2 &size) noexcept;
...
inline void ImGui::FileBrowser::SetWindowSize(const ImVec2 &size) noexcept
{
...
inline void ImGui::FileBrowser::Display()
{
        ...
        if(!BeginPopup(openLabel_.c_str(), ImGuiWindowFlags_NoMove))
        {
            return;
                             ...
                             (flags_ & ImGuiFileBrowserFlags_NoTitleBar ?
                                ImGuiWindowFlags_NoTitleBar : 0) | ImGuiWindowFlags_NoMove))
    {
        return;
    }
        ...
        reserveHeight += GetFrameHeightWithSpacing();
    {
        ...
        for(auto &rsc : fileRecords_)
        {
            ...
            if (IsMouseReleased(0) && IsItemHovered(ImGuiHoveredFlags_None))
            {
                if(rsc.isDir)
                {
                    setNewPwd = true;
...
inline void ImGui::FileBrowser::UpdateFileRecords()
{
    ...
    std::sort(fileRecords_.begin(), fileRecords_.end(),
        [](const FileRecord &L, const FileRecord &R) -> bool
    {
